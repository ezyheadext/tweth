...
async function readWriteAsync(name,bid) {
  ...
  const converter = createConverter({headless:true, args:['--no-sandbox']});
  try {
    await converter.convertFile(inputFilePath);
  } finally {
...
async function checkOffers(i){
  try{
      ...
      for (let i = 0; i < responseEvents.result.data.length; i++) {
          if( responseEvents.result.data[i].ens != undefined && responseEvents.result.data[i].ens.name.startsWith("0x") &&  responseEvents.result.data[i].price >= MINIMUM_OFFER){
...
async function checkSales(i){
    try{
        ...
        for (let i = 0; i < responseEvents.result.data.length; i++) {
            ...
            // added sale >= MINIMUM_SALE && 
            if(sale >= MINIMUM_SALE && !text.includes(null) && name.startsWith("0x") && !tweetlist.includes(text)){
                tweetlist.push(text)   
                writeFile(text)
